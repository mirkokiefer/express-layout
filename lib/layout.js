...
(function() {
  ...
  render = function(name, views, renderFun, cb) {
    var data, mapRequired, renderRequired, requires, view;
    view = views[name];
    data = view.data ? view.data : function(cb) {
      return cb(null, {});
    };
    requires = view.requires ? view.requires : [];
    renderFun = view.renderFun ? view.renderFun : renderFun;
    mapRequired = function(viewName, cb) {
      return render(viewName, views, renderFun, cb);
    };
    ...
    return async.parallel([data, renderRequired], function(err, _arg) {
